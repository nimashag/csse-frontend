...
const UpdateHospital: React.FC = () => {
  ...
  // Fetch hospital data to populate form
  useEffect(() => {
    const fetchHospital = async () => {
      try {
        const response = await fetch(
          `${config.backend_url}/api/hospitals/${id}`
        );
        if (response.ok) {
          const data = await response.json();
          setFormData({
            hospitalName: data.hospitalName,
            hospitalBranch: data.area,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNotification({
        message: "Failed to update hospital. Please try again later.",
        type: "error",
      });
      setTimeout(() => {
        setNotification(null);
      ...
      <main className="main-content">
        <header className="header">
          <div className="header-left">
            <h2 className="text-3xl font-semibold">Update Hospital</h2>
          </div>
        </header>
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-2 gap-6">
              <div className="form-group">
                ...
                />
                {errors.hospitalName && (
                  <span className="text-red-500">{errors.hospitalName}</span>
                )}
              </div>
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onBlur={handleBlur}
                  className={`form-input ${
                    errors.hospitalBranch ? "border-red-500" : ""
                  }`}
                  placeholder="Enter Hospital Branch"
                  required
                />
                {errors.hospitalBranch && (
                  <span className="text-red-500">{errors.hospitalBranch}</span>
                )}
              </div>
              ...
              <div className="form-group">
                ...
                {errors.contactNumber && (
                  <span className="text-red-500">{errors.contactNumber}</span>
                )}
              </div>
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onBlur={handleBlur}
                  className={`form-input ${
                    errors.hospitalEmail ? "border-red-500" : ""
                  }`}
                  placeholder="Enter Email"
                  required
                />
                {errors.hospitalEmail && (
                  <span className="text-red-500">{errors.hospitalEmail}</span>
                )}
              </div>
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onBlur={handleBlur}
                  className={`form-input ${
                    errors.hospitalType ? "border-red-500" : ""
                  }`}
                  required
                >
                ...
                {errors.hospitalType && (
                  <span className="text-red-500">{errors.hospitalType}</span>
                )}
              </div>
            </div>
